...
> **AI Context**: Documentación completa de los módulos de helpdesk de OCA para Odoo 19, incluyendo gestión de tickets, SLA, integración con proyectos y timesheet. Estos módulos reemplazan al Odoo Helpdesk Enterprise y proporcionan funcionalidades ITSM completas integradas con NetBox (CMDB) y Wazuh (SIEM) en NEO_NETBOX_ODOO_STACK. Referencia para implementación de service desk con control de SLA, escalamiento automático y seguimiento de tiempo.

## Visión General

Los módulos de **Helpdesk OCA** proporcionan un sistema completo de **gestión de tickets** y **service desk** para Odoo 19, reemplazando el módulo Enterprise con funcionalidades equivalentes o superiores.

### Módulos Incluidos

| Módulo | Versión | Función | Estado |
|--------|---------|---------|--------|
| helpdesk_mgmt | 19.0.1.0.0 | Gestión básica de tickets | Activo |
| helpdesk_mgmt_sla | 19.0.1.0.0 | SLA y deadlines | Activo |
| helpdesk_type_sla | 19.0.1.0.0 | SLA por tipo de ticket | Activo |
| helpdesk_mgmt_project | 19.0.1.0.0 | Integración con proyectos | Activo |
...
```bash
# Reiniciar contenedor con nuevo addons_path
docker-compose restart odoo

# Instalar módulos vía CLI
...
- Gestión de tickets con workflow personalizable
- Múltiples equipos (teams) de atención
- Etapas personalizadas por equipo
- Categorías y tipos de tickets
...
# Crear equipo de Seguridad (para alertas Wazuh)
team_security = env['helpdesk.ticket.team'].create({
    'name': 'Security Operations',
    'alias_name': 'security',
    'use_sla': True,
    'default_priority': '3',  # Alta prioridad
})
...
```python
# Etapas para Service Desk
stages = [
    {'name': 'Nuevo', 'sequence': 1, 'is_close': False},
    {'name': 'En Análisis', 'sequence': 2, 'is_close': False},
    {'name': 'En Progreso', 'sequence': 3, 'is_close': False},
    {'name': 'Esperando Cliente', 'sequence': 4, 'is_close': False},
    {'name': 'Resuelto', 'sequence': 5, 'is_close': True},
...
class CustomTicketCreator(models.Model):
    ...
    def create_ticket_from_email(self, subject, body, email_from):
        """Crea ticket a partir de email recibido"""
...
- Definición de SLA por equipo
- Cálculo automático de deadlines
- Alertas de violación de SLA
- Reportes de performance de SLA
- Pausar/Reanudar conteo de SLA

### Configuración de SLA

```python
# Crear SLA para Service Desk
...
# SLA para tickets baja prioridad
sla_low = SLA.create({
    'name': 'Baja - 72 horas',
    'team_id': team_servicedesk.id,
    'priority': '1',
...
class HelpdeskSLAAlert(models.Model):
    ...
    def _cron_check_sla_violations(self):
        """Ejecutado cada hora para verificar violaciones de SLA"""
        Ticket = self.env['helpdesk.ticket']
        now = fields.Datetime.now()
        warning_threshold = timedelta(hours=2)

        # Tickets con SLA próximo a expirar (2h)
    ...
    def _escalate_ticket(self, ticket):
        """Escala ticket con SLA violado"""
        # Aumentar prioridad
        if ticket.priority != '3':
            ticket.priority = '3'

        # Notificar gerente del equipo
        if ticket.team_id.manager_id:
            ticket.message_post(
                body=f"SLA VIOLADO! Ticket #{ticket.number} superó el deadline.",
                partner_ids=[ticket.team_id.manager_id.partner_id.id],
                message_type='notification',
            )

        # Crear log de violación
...
- Soporte a múltiples SLAs por ticket
- SLA con condiciones (categoría, prioridad, tipo)

### Configuración de SLA por Tipo
...
# SLA para solicitudes
SLA.create({
    'name': 'Solicitud Normal - 48h',
    'team_id': team_servicedesk.id,
...
- Conversión de tickets en tareas de proyecto
- Vinculación de tickets a proyectos existentes
- Sincronización de estado ticket/tarea
- Visibilidad de tareas en el ticket

### Convertir Ticket en Tarea
...
class HelpdeskTicket(models.Model):
    ...
    def action_convert_to_task(self):
        """Convierte ticket en tarea de proyecto"""
        self.ensure_one()

        Task = self.env['project.task']

        # Buscar proyecto por defecto del equipo
        project = self.team_id.default_project_id
        if not project:
            raise UserError('Configure un proyecto por defecto para el equipo.')
        ...
        # Actualizar ticket
        self.message_post(
            body=f"Ticket convertido en tarea: <a href='/web#id={task.id}&model=project.task'>{task.name}</a>"
        )
...
- Registro de horas trabajadas en tickets
- Reportes de tiempo por técnico/equipo
- Integración con facturación (opcional)
- Control de horas estimadas vs reales

### Registrar Horas en el Ticket

```python
# Agregar timesheet al ticket
...
class HelpdeskTicket(models.Model):
    ...
    def action_add_timesheet(self):
        """Abrir wizard para agregar horas"""
        return {
            'type': 'ir.actions.act_window',
            'name': 'Agregar Horas',
...
### Crear Tickets Automáticamente de Alertas
...
class WazuhHelpdeskIntegration(models.Model):
    ...
    def webhook_receive_alert(self, alert_data):
        """Recibe webhook de Wazuh y crea ticket"""
        Ticket = self.env['helpdesk.ticket']

        # Solo alertas con severidad >= 7
    ...
    def _format_alert_description(self, alert_data):
        """Formatea descripción del ticket a partir de la alerta"""
...
**Regla**: {rule.get('description')}
**ID de la Regla**: {rule.get('id')}
**Nivel**: {rule.get('level')}
...
**Timestamp**: {alert_data.get('timestamp')}
    ...
    def _get_security_category(self):
        """Retorna categoría de seguridad"""
...
```python
# Agregar campo de device al ticket
from odoo import models, fields
...
class HelpdeskTicket(models.Model):
    ...
    def _fetch_netbox_device(self, device_id):
        """Busca información del device en NetBox"""
        import os
...
- **Repositorio OCA**: https://github.com/OCA/helpdesk
- **Documentación**: https://github.com/OCA/helpdesk/tree/19.0
- **Issues**: https://github.com/OCA/helpdesk/issues
- **Comunidad**: https://discuss.odoo.com/c/oca/helpdesk

---

**Siguiente**: [Project Management](project.md)
