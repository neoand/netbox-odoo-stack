...
> **AI Context**: Documentación completa de los módulos de reporting de OCA para Odoo 19, incluyendo exportação Excel/CSV, query builder SQL e BI. Estos módulos proporcionan herramientas avanzadas para creación de reportes, análisis e exportación de datos en NEO_NETBOX_ODOO_STACK. Referencia para creación de reportes personalizados, dashboards e exportaciones automatizadas.

## Visión General

Los módulos **Reporting Engine OCA** proporcionan herramientas avanzadas para creación de **reportes**, **análisis** e **exportación de datos** en Odoo 19.

### Módulos Incluidos

| Módulo | Versión | Función | Status |
|--------|--------|--------|--------|
| report_xlsx | 19.0.1.0.0 | Reportes em Excel | Activo |
| report_csv | 19.0.1.0.0 | Reportes em CSV | Activo |
| bi_sql_editor | 19.0.1.0.0 | Query builder SQL visual | Activo |
| sql_export_excel | 19.0.1.0.0 | SQL direto para Excel | Activo |
| report_qweb_pdf | 19.0.1.0.0 | PDFs personalizados | Activo |
...
- Exportação de datos para Excel (.xlsx)
- Formatação rica (cores, fontes, borders)
- Múltiplas abas
- Fórmulas Excel
- Gráficos e tabelas dinâmicas

### Crear Relatório XLSX Básico
...
class TicketReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, tickets):
        """Generar reporte Excel de tickets"""
        ...
        # Cabeçalhos
        headers = [
            'Número', 'Título', 'Equipo', 'Estágio', 'Prioridad',
            'Responsable', 'Cliente', 'Data Criação', 'Data Fechamento',
            'Tempo Resolução (h)'
        ]
...
class TicketMultiSheetReportXlsx(models.AbstractModel):
    ...
    def _generate_charts_sheet(self, workbook, tickets):
        ...
        # Crear gráfico de pizza: tickets por prioridade
...
class TicketReportCsv(models.AbstractModel):
    ...
    def generate_csv_report(self, data, tickets):
        """Generar reporte CSV de tickets"""
...
- Crear views materializadas
- Reportes baseados em SQL
- Agendamento de atualização
- Exportação para Excel/CSV

### Crear Query SQL Customizada
...
```python
# Crear cron job para atualizar view materializada
Cron = env['ir.cron']
...
class SqlExportExcel(models.TransientModel):
    ...
    def action_export(self):
        """Ejecutar query e exportar para Excel"""
        self.ensure_one()
...
class NetboxDeviceReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, devices):
        ...
        # Cabeçalhos
        headers = ['Name', 'Device Type', 'Site', 'Rack', 'Serial', 'Estado', 'Primary IP', 'Last Updated']
        for col, header in enumerate(headers):
            sheet.write(0, col, header, header_format)
...
- **Repositorio OCA**: https://github.com/OCA/reporting-engine
...
**Siguiente**: [Server UX](server-ux.md)
