...
> **AI Context**: Documentação completa dos módulos REST Framework da OCA para Odoo 19, incluindo FastAPI, autenticação por API key, queue jobs e Pydantic. Estes módulos fornecem infraestrutura moderna de APIs REST para integração do Odoo com sistemas externos como NetBox e Wazuh no NEO_NETBOX_ODOO_STACK. Referência para criação de endpoints REST, processamento assíncrono e validação de dados.

## Visão Geral

Os módulos **REST Framework OCA** fornecem infraestrutura moderna para criar **APIs REST** no Odoo 19, utilizando **FastAPI** como framework principal e incluindo sistema de **filas assíncronas** e **validação de dados**.

### Módulos Incluídos

| Módulo | Versão | Função | Status |
|--------|--------|--------|--------|
| fastapi | 18.0.1.3.1 | Framework FastAPI no Odoo | Ativo |
| fastapi_auth_api_key | 18.0.1.0.0 | Autenticação por API Key | Ativo |
| queue_job | 19.0.1.0.0 | Processamento assíncrono | Ativo |
| pydantic | 19.0.1.0.0 | Validação de schemas | Ativo |
...
```bash
# Atualizar requirements.txt do container
...
# Criar API Key para integração NetBox
User = env['res.users']
...
```python
# Usar dependency de autenticação
...
def create_ticket(
    ...
    """Criar ticket (requer scope tickets:write)"""
    # Implementation...
    pass
```
...
### Converter Odoo para Pydantic

```python
# Helper para conversão
def odoo_ticket_to_pydantic(ticket) -> TicketDetail:
    """Converte registro Odoo para schema Pydantic"""
    return TicketDetail(
        id=ticket.id,
        number=ticket.number,
        name=ticket.name,
...
def receive_wazuh_alert(
    ...
    """Receber alerta do Wazuh e criar ticket"""
...
- **Repositório OCA**: https://github.com/OCA/rest-framework
...
**Próximo**: [Reporting Engine](reporting-engine.md)
