...
> **AI Context**: Documentação completa dos módulos Server UX da OCA para Odoo 19, incluindo anúncios, ações em massa, filtros customizados e validações multi-nível. Estes módulos melhoram a experiência do usuário e produtividade na interface do Odoo no NEO_NETBOX_ODOO_STACK. Referência para customização de interface, automações e workflows de aprovação.

## Visão Geral

Os módulos **Server UX OCA** fornecem melhorias na **experiência do usuário** e **produtividade** da interface do Odoo 19.

### Módulos Incluídos

| Módulo | Versão | Função | Status |
|--------|--------|--------|--------|
| announcement | 19.0.1.0.0 | Anúncios no sistema | Ativo |
...
| mass_editing | 19.0.1.0.0 | Edição em lote avançada | Ativo |
...
maintenance_announcement = Announcement.create({
    ...
    'content': """
        ...
        <p>O sistema NetBox passará por manutenção no dia <strong>15/12/2025</strong> das 02:00 às 04:00.</p>
    ...
    'user_ids': [],  # Vazio = todos os usuários
    'in_date': True,
})
...
```python
# Criar anúncio apenas para equipe de helpdesk
...
- Atualizar campos em lote
- Validações customizadas

### Criar Ação de Edição em Massa
...
```python
# Criar server action via código
...
# Ação: Fechar tickets resolvidos
...
```python
# Ação com validação de permissão
...
```python
# Adicionar tier validation a tickets
from odoo import models, fields, api
...
```python
# Adicionar métodos de aprovação
class HelpdeskTicket(models.Model):
...
```python
# Criar configuração de mass edit
MassEdit = env['mass.object']

# Configuração para tickets
...
```python
# Criar anúncio automaticamente quando alerta crítico é recebido
...
- **Repositório OCA**: https://github.com/OCA/server-ux
- **Documentação**: https://github.com/OCA/server-ux/tree/19.0

---

**Próximo**: [Stock Logistics](stock-logistics.md)
