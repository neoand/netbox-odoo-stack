...
> **AI Context**: Documentação completa dos módulos Stock Logistics da OCA para Odoo 19, incluindo inventário, zonas de localização e movimentação de estoque. Estes módulos fornecem funcionalidades avançadas de gestão de estoque integradas com manutenção e equipamentos no NEO_NETBOX_ODOO_STACK. Referência para controle de inventário de TI, peças de reposição e ativos físicos.

## Visão Geral

Os módulos **Stock Logistics OCA** fornecem funcionalidades avançadas de **gestão de estoque** e **armazenamento** para Odoo 19.

### Módulos Incluídos

| Módulo | Versão | Função | Status |
|--------|--------|--------|--------|
| stock_inventory | 19.0.1.0.0 | Inventário de estoque | Ativo |
| stock_location_zone | 19.0.1.0.0 | Zonas de localização | Ativo |
| stock_move_location | 19.0.1.0.0 | Movimentação facilitada | Ativo |
| stock_storage_type | 19.0.1.0.0 | Tipos de armazenamento | Ativo |
...
### Criar Produtos de TI

```python
# Criar produtos para inventário de TI
...
# Gerar linhas de inventário
inventory.action_start()

# Atualizar quantidades contadas
...
```python
# Criar zonas de armazenamento
Zone = env['stock.location.zone']

# Zona A: Equipamentos Ativos
zone_a = Zone.create({
...
```python
# Relatório de estoque por zona
def get_stock_by_zone(zone_id):
    """Obter estoque agrupado por zona"""
    Zone = env['stock.location.zone']
...
```python
# Vincular produtos a equipamentos
from odoo import models, fields
...
```python
# Sincronizar dispositivos do NetBox para estoque
import requests
import os
...
class NetboxStockSync(models.Model):
    ...
    def sync_devices_to_stock(self):
        """Sincronizar dispositivos do NetBox para produtos"""
...
```python
# Encontrar produtos com estoque negativo
Quant = env['stock.quant']
...
- **Repositório OCA**: https://github.com/OCA/stock-logistics-warehouse
- **Documentação**: https://github.com/OCA/stock-logistics-warehouse/tree/19.0

---

**Próximo**: [Maintenance](maintenance.md)
