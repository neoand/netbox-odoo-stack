...
<template>
  <div class="api-layer-demo p-6 space-y-8">
    <div class="flex items-center justify-between">
      <h1 class="text-3xl font-bold">API Layer Demo</h1>
      <div class="flex items-center gap-4">
        <span class="text-sm text-gray-500">
          {{ t('common.theme') }}: {{ effectiveTheme }}
        </span>
        <UButton @click="toggleMode()" variant="outline" size="sm">
    ...
    <UCard>
      <template #header>
        <h2 class="text-xl font-semibold">{{ t('auth.login') }} Status</h2>
      </template>
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-semibold">{{ t('nav.users') }}</h2>
          <UButton @click="refreshUsers" :loading="usersLoading" size="sm">
            {{ t('common.refresh') }}
          </UButton>
        </div>
      </template>
      ...
      <div class="space-y-4">
        ...
        <UButton @click="createUser" class="w-full">
    ...
    <UCard>
      <template #header>
        <h2 class="text-xl font-semibold">{{ t('common.search') }}</h2>
      </template>
    ...
    <UCard>
      <template #header>
        <h2 class="text-xl font-semibold">{{ t('validation.title') }}</h2>
      </template>

      <form @submit.prevent="submitForm" class="space-y-4">
        <UFormGroup :label="t('auth.name')" :error="formErrors.name?.[0]">
          <UInput v-model="form.name" />
        </UFormGroup>

        <UFormGroup :label="t('auth.email')" :error="formErrors.email?.[0]">
          <UInput v-model="form.email" type="email" />
        </UFormGroup>

        <UFormGroup :label="t('auth.password')" :error="formErrors.password?.[0]">
          <UInput v-model="form.password" type="password" />
        </UFormGroup>

        <UFormGroup :label="t('validation.age')" :error="formErrors.age?.[0]">
          <UInput v-model="form.age" type="number" min="18" />
        </UFormGroup>
    ...
    <UCard>
      <template #header>
        <h2 class="text-xl font-semibold">{{ t('common.helpers') }}</h2>
      </template>
    ...
    <UCard>
      <template #header>
        <h2 class="text-xl font-semibold">{{ t('common.toasts') }}</h2>
      </template>

      <div class="grid grid-cols-2 gap-2">
        <UButton @click="success('Success!', 'This is a success message')" color="green">
          {{ t('common.success') }}
        </UButton>

        <UButton @click="error('Error!', 'This is an error message')" color="red">
          {{ t('common.error') }}
        </UButton>

        <UButton @click="warning('Warning!', 'This is a warning message')" color="yellow">
          {{ t('common.warning') }}
        </UButton>

        <UButton @click="info('Info!', 'This is an info message')" color="blue">
          {{ t('common.info') }}
        </UButton>
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-semibold">{{ t('common.pagination') }}</h2>
      ...
      <div class="space-y-4">
        <div v-if="paginatedLoading" class="space-y-2">
          <USkeleton v-for="i in 5" :key="i" class="h-12 w-full" />
        </div>

        <div v-else-if="paginatedData" class="space-y-2">
          <div
