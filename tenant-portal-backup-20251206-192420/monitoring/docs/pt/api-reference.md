...
O **Monitoring Service** é responsável pela observabilidade completa da plataforma NEO_STACK v3.0. Coleta métricas, logs e traces de todos os microservices, fornecendo dashboards em tempo real, alerting e analytics usando Prometheus + Grafana + AlertManager.

### Informações Base

- **Versão da API**: v3.0
- **Base URL**: `https://monitoring.platform.local/api/v3`
...
| metric | string | Não | Filtrar por métrica |
| start | string | Não | Data inicial |
| end | string | Não | Data final |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|-----------|------|-------------|-----------|
| dashboard_name | string | Sim | Nome do dashboard |
...
| Parâmetro | Tipo | Obrigatório | Descrição |
|-----------|------|-------------|-----------|
| query | string | Sim | Query PromQL |
| time | string | Não | Timestamp |
...
1. **platform-overview**: Visão geral da plataforma
2. **api-gateway**: Métricas do API Gateway
3. **auth-service**: Métricas do serviço de autenticação
4. **tenant-manager**: Métricas do gerenciador de tenants
5. **infrastructure**: Métricas de infraestrutura
